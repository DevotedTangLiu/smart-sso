...
<div class="row">
	<div class="col-xs-12">
		...
		<form id="_editForm" class="form-horizontal" role="form"
				...
				<div class="col-sm-3">
					<select id="_appId" name="appId" class="form-control help-validate"
						...
						<c:forEach var="item" items="${appList}">
							<option value="${item.id}" ${(item.id eq role.appId || item.id eq appId)?'selected="selected"':''}>${item.name}</option>
						</c:forEach>
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				<label for="_sort" class="col-sm-3 control-label no-padding-right"><span class="form-star">*</span>优先级</label>

				<div class="col-sm-6">
					<p id="_warning" class="_warning" style="display:none; color: orange">优先级等级分为1-5，其中1为最低，5为最高。</p>
					<div class="clearfix help-validate">
						<input id="_sort" name="sort" type="text" value="${role.sort}" class="form-data col-xs-10 col-sm-5" placeholder="请输入值"
							   required="true" vtype="integer" min="1" max="5"/>
					</div>
				</div>
                ...
                if($('#_editForm').validate()){
                    ...
                    $.post("${_path}/admin/role/save", $.formJson('_editForm'),function(d) {
                        if(d){
                            btn.button('reset');
                            if(d.code == 1){
                                $.aceRedirect("${_path}/admin/role?appId="+$("#_appId option:selected").val());
                            }
                            else {
