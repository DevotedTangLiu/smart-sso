...
public class SsoFilter extends ClientFilter {
    ...
    private String getParameterToken(HttpServletRequest request) throws IOException {
        ...
        if (token != null) {
            ...
            if (rpcUser != null) {
                invokeAuthenticationInfoInSession(request, token, rpcUser.getUserId(), rpcUser.getAccount());
                return token;
            }
        }
        return null;
     ...
     * @param account
     */
    private void invokeAuthenticationInfoInSession(HttpServletRequest request, String token, Integer userId, String account) {
        SessionUtils.setSessionUser(request, new SessionUser(token, userId, account));
    }
